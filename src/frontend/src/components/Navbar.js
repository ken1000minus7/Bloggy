...
export const Navbar = ({ changeTheme }) => {
  ...
  return (
    <div className="font-SourceSansPro flex flex-row items-center shadow-[rgba(99,99,99,0.2)_0_2px_8px_0] bg-[#ede7f6]">
      <Link
        to="/home"
        className="font-bold m-[10px] text-[36px] font-serif sm:m-[5px] sm:text-[25px] flex flex-row items-center"
      >
        <img src={logo} alt={"Bloggy"} className="h-[38px] sm:h-[30px]" />
        {width > 640 && <div className="ml-[5px] font-itim">Bloggy</div>}
      </Link>
      <div className="flex flex-auto flex-row mx-[10px] sm:mx-0 sm:justify-evenly">
        <NavbarItem
          ...
          title="Home"
          icon={
            <Home fontSize={width > 830 || width < 600 ? "medium" : "large"} />
          }
        />
        <NavbarItem
          to="/search"
          ...
          icon={
            <Search
              fontSize={width > 830 || width < 600 ? "medium" : "large"}
            />
          }
        />
        <NavbarItem
          ...
          title="Create"
          icon={
            <Add fontSize={width > 830 || width < 600 ? "medium" : "large"} />
          }
        />
      </div>
      ...
      {username ? (
        <div>
          ...
          >
            <div className="text-[18px] mx-[5px] sm:hidden">{username}</div>
            <Avatar className="mx-[5px] sm:mx-0" />
          </Button>
          <Menu open={menuOpen} onClose={changeMenuOpen} anchorEl={anchor}>
            <MenuItem
              onClick={() => {
                setMenuOpen(false);
        ...
        <Link
          to={"/login"}
          className="font-bold text-[20px] my-[10px] mx-[20px] duration-[400ms] hover:opacity-[0.5] sm:mx-[5px] sm:text-[15px] font-SourceSansPro"
        >
          Login
        </Link>
      )}
